 ...
 */
public class SaveOrPrintLorryActivity extends Activity implements View.OnClickListener {
    ...
    private Handler handler = new Handler(new Handler.Callback() {
        ...
        public boolean handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case 1:
                    Toast.makeText(SaveOrPrintLorryActivity.this, R.string.tip_data_success_save, Toast.LENGTH_SHORT).show();
                    if (dialog != null) dialog.cancel();
                    break;
                case 2:
                    Toast.makeText(SaveOrPrintLorryActivity.this, R.string.tip_data_printing, Toast.LENGTH_SHORT).show();
                    if (dialog != null) dialog.cancel();
                    break;
                case 3:
                    Toast.makeText(SaveOrPrintLorryActivity.this, R.string.tip_data_fail_save, Toast.LENGTH_SHORT).show();
                    if (dialog != null) dialog.cancel();
                    break;
    ...
    private void init() {
        ...
        } else if (MainActivityLorry.lorryReferData != null) {
            et_startYear.setText(MainActivityLorry.lorryReferData.getStartYear());
            et_endYear.setText(MainActivityLorry.lorryReferData.getEndYear());
            et_model.setText(MainActivityLorry.lorryReferData.getVehicleInfo());
            et_manufacturer.setText(MainActivityLorry.lorryReferData.getManInfo());
        }
    }
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.btn_save:
                ...
                } else if (MainActivityLorry.printData == null) {
                    Toast.makeText(this, R.string.tip_data_cannot_null, Toast.LENGTH_SHORT).show();
                } else if (MainActivityLorry.printData.isSaved() && !isPrint) {
    ...
    class DataSaveOrPrint implements Runnable {
        ...
        public void run() {
            ...
            if (MainActivityLorry.lorryReferData.getWheelbase() != null) {

                vehicleInfo.setWheelBase(MainActivityLorry.lorryReferData.getWheelbase().getMid());
                vehicleInfo.setFrontTread(MainActivityLorry.lorryReferData.getFrontWheel().getMid());
                vehicleInfo.setRearTread(MainActivityLorry.lorryReferData.getRearWheel().getMid());
            }
            ...
            if (isPrint) {
                ...
                new Thread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        client.send(MainApplication.printContent, MainActivityLorry.printData.getPrintData(customerInfo, vehicleInfo));
                        client.onStop();
                        handler.sendEmptyMessage(2);
                    }
                }).start();

            } else if (!MainActivityLorry.printData.isSaved()) {
                //保存数据
                ...
                flag &= util.addLoryyTestData(MainActivityLorry.printData, customerInfo.getId(), vehicleInfo.getPlateNum(), vehicleInfo.getTestSerial(), time);

                new Thread(new Runnable() {
                    @Override
                    public void run() {
                        SocketClient client = new SocketClient(null, false);
                        client.send(MainApplication.hostSaveDataUrl, MainActivityLorry.printData.getPrintData(customerInfo, vehicleInfo));
                        client.onStop();
